...
import { useAuth } from "@/contexts/auth-context"; // 🛡️ Nossa Bolha de Segurança
...
export default function RegistrosPage() {
  // 1. Puxamos as credenciais e perfil
  ...
  const fetchRegistros = async () => {
    try {
      ...
      // 🔒 REGRA DE SEGURANÇA: Filtra por estado se for usuário externo
  ...
  useEffect(() => {
    // Só faz a busca após validar quem é o usuário
    if (!authLoading) {
      fetchRegistros();
    }
  ...
  return (
    <div className="h-screen w-full bg-[#f8fafc] p-4 flex flex-col gap-4 overflow-hidden">
      ...
      <div className="flex items-center justify-between shrink-0">
        ...
        {/* 🔒 BOTÕES RESTRITOS: Só aparecem para Internos */}
      ...
      <div className="bg-white p-2.5 rounded-lg border border-slate-200 flex items-center justify-between shrink-0 shadow-sm">
        <div className="flex items-center gap-3 flex-1">
          <div className="relative w-[300px]">
            ...
            <Input 
              placeholder="Buscar por local ou objeto..." 
      ...
      {/* TABELA EXTENSA: Padrão Oficial do Projeto */}
